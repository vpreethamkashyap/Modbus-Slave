//###########################################################################
// FILE:   softssi.h
// TITLE:  Defines and macros for the SoftSSI.
//###########################################################################
...
typedef struct
{
        ...
        //! The address of the GPIO pin to be used for the Clk signal.  This
        //! member can be set via a direct structure access or using the
        //! SoftSSIClkGPIOSet function.
        unsigned long ulClkGPIO;

        //! The address of the GPIO pin to be used for the Tx signal.  This
        //! member can be set via a direct structure access or using the
        //! SoftSSITxGPIOSet function.
        unsigned long ulTxGPIO;

        //! The address of the GPIO pin to be used for the Rx signal.  If this
        //! member is zero, the Rx signal is not read.  This member can be set
        //! via a direct structure access or using the SoftSSIRxGPIOSet
	//! function.
        unsigned long ulRxGPIO;

        //! The address of the data buffer used for the transmit FIFO.  This
        //! member can be set via a direct structure access or using the
        //! SoftSSITxBufferSet function.
        unsigned short *pusTxBuffer;

        //! The address of the data buffer used for the receive FIFO.  This
        //! member can be set via a direct structure access or using the
        //! SoftSSIRxBufferSet function.
        unsigned short *pusRxBuffer;

        //! The length of the transmit FIFO.  This member can be set via a
        //! direct structure access or using the SoftSSITxBufferSet function.
        unsigned short usTxBufferLen;

        //! The index into the transmit FIFO of the next word to be transmitted.
        //! This member should be initialized to zero, but should not be
        ...
        //! into the FIFO.  This member should be initialized to zero, but
        ...
        //! The index into the receive FIFO of the location to store the next
        //! word received.  This member should be initialized to zero, but 
	//! should not be accessed or modified by the application.
        unsigned short usRxBufferWrite;

        //! The word that is currently being transmitted.  This member should
        //! not be accessed or modified by the application.
        unsigned short usTxData;

        //! The word that is currently being received.  This member should not
        ...
        //! The number of data bits in each SoftSSI frame, which also specifies
        //! the width of each data item in the transmit and receive FIFOs.  This
        //! member can be set via a direct structure access or using the
        //! SoftSSIConfigSet function.
        unsigned char ucBits;

        //! The current state of the SoftSSI state machine.  This member should
        //! not be accessed or modified by the application.
        unsigned char ucState;

        //! The number of bits that have been transmitted and received in the
        //! current frame.  This member should not be accessed or modified by
        //! the application.
        ...
        //! data stored in the receive FIFO, which is used to generate the
        //! receive timeout interrupt.  This member should not be accessed or 
...
//*****************************************************************************
#define SOFTSSI_FRF_MOTO_MODE_0          0x00000000 // Moto fmt, polarity 0,
                                                    // phase 0
#define SOFTSSI_FRF_MOTO_MODE_1          0x00000002 // Moto fmt, polarity 0,
                                                    // phase 1
#define SOFTSSI_FRF_MOTO_MODE_2          0x00000001 // Moto fmt, polarity 1,
                                                    // phase 0
#define SOFTSSI_FRF_MOTO_MODE_3          0x00000003 // Moto fmt, polarity 1,
                                                    // phase 1
