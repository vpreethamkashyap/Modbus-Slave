//###########################################################################
// FILE:   cpu_usage.c
// TITLE:  Routines to determine the CPU utilization.
//###########################################################################
...
CPUUsageTick(void)
{
    ...
    ulValue = TimerValueGet(g_pulCPUUsageTimerBase[g_ulCPUUsageTimer],
                            TIMER_A);
             ...
             unsigned long ulTimer)
{
    ...
    SysCtlPeripheralEnable(g_pulCPUUsageTimerPeriph[ulTimer]);
    SysCtlPeripheralSleepDisable(g_pulCPUUsageTimerPeriph[ulTimer]);

    // Configure the third timer for 32-bit periodic operation.
    TimerConfigure(g_pulCPUUsageTimerBase[ulTimer], TIMER_CFG_32_BIT_PER);

    // Set the load value for the third timer to the maximum value.
    TimerLoadSet(g_pulCPUUsageTimerBase[ulTimer], TIMER_A, 0xffffffff);

    // Enable the third timer.  It will now count the number of system clocks
    // during which the processor is executing code.
    TimerEnable(g_pulCPUUsageTimerBase[ulTimer], TIMER_A);
}
